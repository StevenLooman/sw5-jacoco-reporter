 ...
 *
 * <p>Given a {@link IClassCoverage}, iterates over the methods and sees if this is a "sub-method",
...
public class MethodCoverageMerger {
  ...
  public MethodCoverageMerger(final SwJarAnalyzerAnalyzer libAnalyzer) {
  ...
  public Collection<IMethodCoverage> run(
    ...
    final ClassNode supplierClassNode =
        subsidiaryClassCoverage != null ? this.getClassNode(subsidiaryClassCoverage) : null;
  ...
  private IMethodCoverage mergeMethodCoverage(
      final IMethodCoverage parentMethodCoverage, final IMethodCoverage childMethodCoverage) {
                ...
                .map(
                    methodCoverage -> {
                      ...
                      final MethodNode methodNode =
                          ClassNodeHelper.getMethodNode(providerClassNode, methodName);
            ...
            subsidiaryClassCoverage.getMethods().stream()
                .map(
                    methodCoverage -> {
                      final String methodName = methodCoverage.getName();
                      final MethodNode methodNode =
                          ClassNodeHelper.getMethodNode(supplierClassNode, methodName);
                      return Map.entry(methodNode, methodCoverage);
                    }))

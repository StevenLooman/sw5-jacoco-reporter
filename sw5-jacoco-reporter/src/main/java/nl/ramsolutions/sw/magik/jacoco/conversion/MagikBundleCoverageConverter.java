...
import nl.ramsolutions.sw.magik.jaranalyzer.SwJarAnalyzerCodeDefinition;
import nl.ramsolutions.sw.magik.jaranalyzer.helpers.ClassNodeHelper;
 ...
 *
 * <p>Conversion includes: - Methods are merged (__loopbody__ into parent method, etc). - Methods
 * are renamed to Magik-names, where applicable. - Executable classes are discarded, when enabled.
 */
public class MagikBundleCoverageConverter {

  private static final Set<String> PRIMARY_CLASS_METHODS = Set.of("<init>", "preload", "execute");

  private final SwJarAnalyzerAnalyzer libAnalyzer;
  ...
  public MagikBundleCoverageConverter(
      final SwJarAnalyzerAnalyzer libAnalyzer,
  ...
  public IBundleCoverage convert() {
    ...
    final List<IPackageCoverage> newPackages =
        this.bundleCoverage.getPackages().stream()
            .map(this::convert)
  ...
  private IPackageCoverage convert(final IPackageCoverage packageCoverage) {
    ...
    final List<IClassCoverage> newClassCoverages =
        classCoverages.stream()
            .map(classCoverage -> this.convert(classCoverages, classCoverage))
            .filter(Objects::nonNull)
            .collect(Collectors.toList());
    final List<ISourceFileCoverage> newSourceFileCoverages =
        packageCoverage.getSourceFiles().stream()
  ...
  private IClassCoverage convert(
      final Collection<IClassCoverage> classCoverages, final IClassCoverage classCoverage) {
  ...
  private IMethodCoverage convert(
      final IClassCoverage subsidiaryClassCoverage, final IMethodCoverage methodCoverage) {
    ...
    if (this.discardExecutable && isPrimaryClassMethod) {
      return null;
    }
    ...
    final SwJarAnalyzerCodeDefinition element =
        this.libAnalyzer.getElement(javaClassName, javaMethodName);
  ...
  private ISourceFileCoverage convert(
    ...
    // Create a copy of the SourceFileCoverage, but strip everything present from the executable
    // part.
    ...
    final SourceFileCoverageImpl newSourceFileCoverage =
        new SourceFileCoverageImpl(name, packageName);
    ...
    final IClassCoverage relatedClassCoverage =
        classCoverages.stream()

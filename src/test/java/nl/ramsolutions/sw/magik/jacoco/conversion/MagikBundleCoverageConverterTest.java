...
/** Tests for {@link MagikBundleCoverageConverter}. */
class MagikBundleCoverageConverterTest {
  ...
  void testFixtureBundleCoverage() throws IOException {
    ...
    final IClassCoverage primaryClassCoverage0 =
    ...
    final IClassCoverage subsidiaryClassCoverage0 =
        packageCoverage0.getClasses().stream()
            .filter(
                classCoverage ->
  ...
  void testMergeMethods() throws IOException {
    ...
    final IPackageCoverage packageCoverage0 = List.copyOf(bundleCoverage.getPackages()).get(0);
    assertThat(packageCoverage0.getClasses())
        .hasSize(3); // Subsidiary are merged into primary: 5 --> 3.
  ...
  void testMergeRemoveExecutable() throws IOException {
    ...
    final IPackageCoverage packageCoverageOrig0 =
        List.copyOf(bundleCoverageOrig.getPackages()).get(0);

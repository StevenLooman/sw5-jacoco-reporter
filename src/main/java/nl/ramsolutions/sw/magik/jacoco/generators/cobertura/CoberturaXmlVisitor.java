...
public class CoberturaXmlVisitor implements IReportVisitor {
  ...
  private void createRootElement(final IBundleCoverage bundleCoverage) throws IOException {
    this.rootElement =
        new XMLElement("coverage", PUBID, SYSTEM, true, OUTPUT_ENCODING, this.output);
  ...
  private void writePackage(final XMLElement packagesEl, final IPackageCoverage packageCoverage) {
    try {
      ...
      packageCoverage
          .getClasses()
          .forEach(classCoverage -> this.writeClass(classesEl, classCoverage));
    } catch (final IOException exception) {
      throw new IllegalStateException(exception);
    }
  }
  ...
  private void writeClass(final XMLElement classesEl, final IClassCoverage classCoverage) {
    try {
      ...
      classCoverage
          .getMethods()
          .forEach(methodCoverage -> this.writeMethod(methodsEl, methodCoverage));
  ...
  private void writeLine(final XMLElement linesEl, final int lineNo, final ILine line) {
    try {
      ...
      final int hits = instructionCounter.getCoveredCount() > 0 ? 1 : 0;
      lineEl.attr("hits", hits);

      final ICounter branchCounter = line.getBranchCounter();
      final String branch = branchCounter.getTotalCount() > 0 ? "true" : "false";

...
final class Sw5LibDependencyBuilder {
  ...
  public static Map<ClassNode, ClassNode> buildClassDependencyMap(
    ...
    return primaryClassNodes.stream()
        .map(
            primaryClassNode -> {
              final String subsidiaryClassName =
                  Sw5LibDependencyBuilder.getSubsidiaryClassName(primaryClassNode);
              final ClassNode subsidiaryClassNode =
                  subsidiaryClassNodes.stream()
   ...
   *
   * <p>Note that the defined methods are not added as a dependency of the {@code execute()} method.
   *
  ...
  public static Map<MethodNode, MethodNode> buildMethodDependencyMap(
    ...
    final Map<MethodNode, MethodNode> subsidiaryDependencyMap =
        supplierClassNode.methods.stream()
            ...
            .collect(
                Collectors.toMap(
                    ...
                    methodNode -> {
                      final String wantedMethodName =
                          MethodNodeHelper.getParentMethodName(methodNode);
    ...
    final MethodNode executeMethodNode =
        ClassNodeHelper.isPrimaryClassNode(providerClassNode)
